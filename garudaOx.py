...
from googlesearch import search  # Added missing import for `search`
from html.parser import HTMLParser  # Added missing import for `parser`
...
# Check and create configs/config.json if missing
config_path = "configs/config.json"
if not os.path.exists(config_path):
    os.makedirs(os.path.dirname(config_path), exist_ok=True)
    with open(config_path, "w") as config_file:
        config_file.write(json.dumps({"veriphone-api-key": "", "real-email-api-key": ""}))
    print(f"Created missing config file at {config_path}")

configs = json.loads(open(config_path, "r").read())

# Handle missing HOME environment variable and `.cookies` file
home = os.getenv("HOME")
if not home:
    home = os.path.expanduser("~")  # Fallback to the user's home directory
    if not home:
        raise EnvironmentError("HOME environment variable is not set, and user home directory could not be determined.")

cookifile = f"{home}/.cookies"
if not os.path.exists(cookifile):
    os.makedirs(os.path.dirname(cookifile), exist_ok=True)  # Ensure the directory exists
    with open(cookifile, "w") as cookie_file:
        cookie_file.write("")
    print(f"Created missing cookies file at {cookifile}")
...
logo = f"""{b}
   ...
   ┏───────────────────────────────────────────────────────────────────────────────┓
   │                                                                               │
   │                       WELCOME TO G A R U D A  O S I N T                       │
   │                                                                               │
   │                  A Powerful Information Gathering Toolkit                     │
   │                                                                               │
   │                   github.com/GONX729 | @muzzXalmighty                         │
   ┗───────────────────────────────────────────────────────────────────────────────┘
{w}
"""
...
def menu():
    ...
    print(f"""
        ...
        {w}{b}  00{w} Exit          {d} Its Time To Leave ):
        """)
    mainmenu()
...
class Facebook():
    ...
    def user_token(self):
        ...
        find = re.search(r"(EAAA\w+)",x.text)  # Fixed invalid escape sequence
        if find == None:
    ...
    def facedumper(self):
        ...
        cookies = {"cookie": coki}
        try:
            req = requests.get(mbasic.format("/me"), cookies=cookies, verify=False).content
        except RequestException as e:
            exit(r + f"* Error fetching Facebook data: {e}" + w)

        if "mbasic_logout_button" in str(req):
            if "What are you thinking now" in str(req):
                with open(cookifile, "w") as f:
                    f.write(cookies["cookie"])
            else:
                try:
                    parser = HTMLParser()  # Fixed undefined `parser`
                    follow_link = parser.unescape(
                        BeautifulSoup(req, "html.parser")
                        .find("a", string="Indonesian Language")["href"]
                    )
                    requests.get(mbasic.format(follow_link), cookies=cookies)
                except Exception as e:
                    print(r + f"* Warning: {e}" + w)
        else:
